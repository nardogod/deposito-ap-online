    ...
    def create_preference(self, order_id):
        ...
        try:
            ...
            preference_data = {
                ...
                "payer": {
                    "name": order.shipping_full_name.split(' ')[0] if order.shipping_full_name else order.user.first_name,
                    "surname": order.shipping_full_name.split(' ')[-1] if order.shipping_full_name else order.user.last_name,
                ...
                # No arquivo payments/services.py
                "notification_url": f"{os.getenv('SITE_URL')}/api/payments/webhook/",
