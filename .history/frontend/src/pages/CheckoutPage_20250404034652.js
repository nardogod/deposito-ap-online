...
const CheckoutPage = () => {
  ...
  // DEBUG: Log do carrinho para inspecionar a estrutura
  useEffect(() => {
    console.log("Estrutura atual do carrinho:", JSON.stringify(cart, null, 2));
  }, [cart]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Criar pedido - tentar estruturar os dados corretamente
      const items = cart.map(item => {
        // Obter o ID do produto com segurança
      ...
      const orderData = {
        ...
        // Campo adicional obrigatório pelo serializer
        phone: user?.phone || "99999999999" // Phone placeholder (precisamos de um telefone)
      };
      ...
      try {
        ...
        console.error("Detalhes do erro da API:", apiError.response?.data);
        throw new Error(apiError.response?.data?.detail || "Erro na API");
      }
      
    } catch (err) {
      console.error('Erro ao processar checkout:', err);
      setError('Ocorreu um erro ao processar seu pedido. Por favor, tente novamente. ' + err.message);
    } finally {
      setLoading(false);
    }
  };
