...
class MercadoPagoService:
    ...
    def create_preference(self, order_id, coupon_code=None):
        """
        Cria uma preferência de pagamento no Mercado Pago para um pedido específico
        Agora suporta aplicação de cupons de desconto
        """
        try:
            ...
            if coupon_code:
                try:
                    ...
                    # Modelo Coupon pode não estar disponível
                    logger.warning("Modelo Coupon não disponível ao aplicar cupom")
                    pass
                except Exception as e:
                    logger.warning(f"Erro ao aplicar cupom: {str(e)}")
            ...
            if hasattr(order, 'shipping_cost') and order.shipping_cost > 0:
                items.append({
            ...
            # Adicionar desconto como item separado, se aplicável
            if discount_amount > 0:
                items.append({
                    "title": f"Desconto: {coupon_code}",
                    "quantity": 1,
                    "unit_price": -float(discount_amount),  # Valor negativo para desconto
                    "currency_id": "BRL",
                    "category_id": "services"
                })
            ...
            # Atualizar pedido com o ID da preferência e cupom (se aplicado)
            ...
            if coupon and discount_amount > 0:
                ...
                update_fields.extend(['coupon_code', 'discount_amount'])
